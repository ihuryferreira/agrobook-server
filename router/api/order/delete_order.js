...
// *************** DELETE ***************
// Controla todas as entrada de apagar Pedido
router.delete("/api/order/delete_order", async (req, res) => {

    const { filter } = req.body; // RESERVA OS VALORES RECEBIDOS VIA BODY
    const keysValid = [ '_id']; // CRIA UMA LISTA DE KEYS VÁLIDAS
    ...
    const deleteData = await shell_command.commandDeleteData('books', 'pedidos', filter); // EXECUTA A FUNÇÃO QUE EXCLUE O REGISTRO NO MONGODB
